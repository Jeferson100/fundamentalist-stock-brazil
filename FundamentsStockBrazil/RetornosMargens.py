...
class RetornosMargensDataScraper:
    ...
    def obter_dados_tabela(self, navegador, data):
        while True:
            try:
                select_element = navegador.find_element(By.XPATH, '/html/body/div[1]/div[4]/div[2]/div[2]/div[2]/table/thead/tr/th[2]/select')
    ...
    def coletar_dados_financeiros(self, navegador, datas):
        dados = {
            ...
            'retorno_sobre_capital_tangivel_inicial_pre_impostos': [np.nan] * len(datas),
            'retorno_sobre_capital_investido_inicial_pre_impostos': [np.nan] * len(datas),
        ...
        for data in datas:
            while True:
                ...
                if lista_resumo_balanco:
                    break
            dados['retorno_sobre_patrimonio_liquido_inicial'].append(lista_resumo_balanco[1].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_ativo_inicial'].append(lista_resumo_balanco[3].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_bruta'].append(lista_resumo_balanco[5].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_liquida'].append(lista_resumo_balanco[7].replace('R$','').replace(',','.').replace('%','').strip())
            dados['giro_do_ativo_inicial'].append(lista_resumo_balanco[9].replace('R$','').replace(',','.').replace('%','').strip())
            dados['alavancagem_financeira'].append(lista_resumo_balanco[11].replace('R$','').replace(',','.').replace('%','').strip())
            dados['passivo_patrimonio_liquido'].append(lista_resumo_balanco[13].replace('R$','').replace(',','.').replace('%','').strip())
    ...
    def coletar_dados_nao_financeiros(self,navegador, datas):
        ...
        for data in datas:
            while True:
                ...
                if lista_resumo_balanco:
                    break
            dados['retorno_sobre_capital_tangivel_inicial'].append(lista_resumo_balanco[1].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_capital_investido_inicial'].append(lista_resumo_balanco[3].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_capital_tangivel_inicial_pre_impostos'].append(lista_resumo_balanco[5].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_capital_investido_inicial_pre_impostos'].append(lista_resumo_balanco[7].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_patrimonio_liquido_inicial'].append(lista_resumo_balanco[9].replace('R$','').replace(',','.').replace('%','').strip())
            dados['retorno_sobre_ativo_inicial'].append(lista_resumo_balanco[11].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_bruta'].append(lista_resumo_balanco[13].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_liquida'].append(lista_resumo_balanco[15].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_ebit'].append(lista_resumo_balanco[17].replace('R$','').replace(',','.').replace('%','').strip())
            dados['margem_ebitda'].append(lista_resumo_balanco[19].replace('R$','').replace(',','.').replace('%','').strip())
            dados['giro_do_ativo_inicial'].append(lista_resumo_balanco[21].replace('R$','').replace(',','.').replace('%','').strip())
            dados['alavancagem_financeira'].append(lista_resumo_balanco[23].replace('R$','').replace(',','.').replace('%','').strip())
            dados['passivo_patrimonio_liquido'].append(lista_resumo_balanco[25].replace('R$','').replace(',','.').replace('%','').strip())
            dados['divida_liquida_ebitda'].append(lista_resumo_balanco[27].replace('R$','').replace(',','.').replace('%','').strip())
            ...
            if acao in acoes_processadas:
                ...
                if acao in self.setor_financeiro:
                    try:
                        ...
                        df_dados = self.coletar_dados_financeiros(navegador=navegador, datas=datas)  
                    colunas = df_dados.columns[(df_dados.columns != 'giro_do_ativo_inicial')&
                    ...
                    for col in colunas:
                        df_dados[col] = pd.to_numeric(df_dados[col], errors='coerce') / 100
                else:
                    try:
